...
@section('content')
    ...
    <div class="card p-3">
        @foreach ($users as $user)
            @if ($user->user_type==0)
                <h4 class="text-danger"><img src="/storage/app/public/profile_pic/{{$user->profile_pic}}" alt="Please Insert Profile Picture" class="col-md-2"></h4>
            ...
            @if ($user->user_type==0)
                ...
                <h4> Resume: 
                    @if ($user->resume)
                        <a href="/storage/app/public/resume/{{$user->resume}}">Download Resume</a></h4>
                    @else
